...
public class UnityConsulo : MonoBehaviour
{
    ...
    /*[MenuItem("File/Set Debugging Port in Consulo")]
    static void SetDebuggingPortInConsulo()
    {
        if (!IsOnAMac())
        {
    ...
    static void CallConsulo(string args)
    {
        ...
        proc.Start();
        //proc.Start("consulo " + args);
        //Process.Start("consulo " + args);
    }
    ...
    static void ParseOutput(string output)
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (port > -1)
            {
                // Port found so put into the build config of Consulo
                WritePortToConsuloConfig(port);
                return;
            }
        }
    }
    ...
    static void WritePortToConsuloConfig(int port)
    {
        UnityEngine.Debug.Log("Current mono debugging port is " + port);
        //  UnityEngine.Debug.Log("Writing Port to Consulo config...");
